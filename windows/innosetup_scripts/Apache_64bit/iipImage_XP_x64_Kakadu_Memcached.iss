; INSTALLER for IIPIMAGE JPEG2000 x64 with Kakadu and Memcached for WinXP
...
; because it looks for apache version 2.2.22 in registry and also extract that file
[Setup]
...
OutputBaseFileName=IIPImage_Apache_x64
...
[Files]
;Apache server
Source: "apache\apache_2.2.22-x64-no-ssl.msi";  Flags: dontcopy
...
Source: "Jpeg2000Transcoder\msvcp90.dll"; DestDir: "{code:GetTranscoderDir}\Jpeg2000Transcoder"
Source: "Jpeg2000Transcoder\msvcr90.dll"; DestDir: "{code:GetTranscoderDir}\Jpeg2000Transcoder"
...
Source: "ProgramFiles\msvcp90.dll"; DestDir: "{code:GetMemcachedDir}"
Source: "ProgramFiles\msvcr90.dll"; DestDir: "{code:GetMemcachedDir}"
;Temporary files - first set memcached, second apache, third creates urls and configres transcoder
...
MsgBox('Installation continue:' #13#13 'The previous IIPImage installation was removed. '
  ...
  begin
    ...
    begin
      ExtractTemporaryFile('apache_2.2.22-x64-no-ssl.msi');
      if not Exec('msiexec',ExpandConstant('/i "{tmp}\apache_2.2.22-x64-no-ssl.msi"'), '', SW_SHOWNORMAL, ewWaitUntilTerminated, ResultCode) then
      begin
