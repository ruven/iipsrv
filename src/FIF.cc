...
void FIF::run( Session* session, const string& src ){
  ...
  try {
    ...
    if ( max_images_in_metadata_cache > 0 && session->imageCache->find(argument) != session->imageCache->end() ) {
      ...
      if ( max_images_in_metadata_cache > 0 ) {
        if ( session->imageCache->empty() ) { // cache is empty
          ...
          if ( session->imageCache->size() >= max_images_in_metadata_cache ) 
	...
	*(session->logfile) << "FIF :: Image timestamp changed: reloading metadata" << endl;
    ...
    // save the ICC Profile to the session if we're supposed to retain it and it hasn't already been set
    // TIFF and Kakadu will have it set by now but the open jpeg library requires a decode before we have access to 
    // the ICC color profile so that gets set in CVT rather than FIF
    if ( session->retain_source_icc_profile != 0 && session->icc_profile_buf == NULL )
